...
def get_next_number(cls):
    def _get_last_number():
        ...
        extens = [
            k['exten'] for k in cls.env['asterisk_base_queues.queue'].search_read(
                [], fields={'exten'}) if
            len(k['exten']) == length]
        if not extens:
            return start
...
class Queue(models.Model):
    ...
    exten = fields.Char(default=get_next_number, related='extension.number', readonly=False,
                        store=True)
    # Queue cmd options
    ...
    def build_conf(self):
        ...
        for rec in self.env['asterisk_base_queues.queue'].search([]):
            ...
            if rec.server.id not in conf_dict:
                conf_dict[rec.server.id] = self.env['asterisk_base.conf_template'].render_template(
                    'general_queue_settings', vals={'rec': server_queue_settings}, server_id=rec.server.id)
                # Adding newline to general conf dict
                conf_dict[rec.server.id] = conf_dict[rec.server.id] + '\n'
            # Build queues
            static_members = [k.interface for k in rec.static_members]
            conf_dict[rec.server.id] += self.env['asterisk_base.conf_template'].render_template(
                'queue',
                vals={'rec': rec, 'static_members': static_members},
    ...
    def join(self):
        for rec in self:
            ...
            if rec.is_user_joined:
                ...
                continue
            logger.info("Adding user {} to Queue {}".format(
                self.env.uid, rec.name))
            self.server.agent.action({
                    'Action': 'QueueAdd',
                    'Queue': rec.name,
                    'Interface': rec.server.asterisk_user.queue_interface
                },
                callback=('asterisk_base_queues.queue', 'join_response'),
                passback={
                    'uid': self.env.uid,
                    'queue': rec.id,
    ...
    def join_response(self, data):
        ...
        def _add_dynamic_operator(this_object, queue_id, asterisk_user_id, uid):
            ...
            self.server.agent.reload_view(
                uid=uid,
                model='asterisk_base_queues.queue')

        # Case 1: Result = False
        uid = data['passback']['uid']
        try:
            res = data['result'][0]
        except TypeError:
    ...
    def leave_response(self, data):
        queue = data.get('passback', {}).get('queue')
        user = data.get('passback', {}).get('asterisk_user')
        uid = data['passback']['uid']
        try:
            res = data['result'][0]
        except TypeError:
    ...
    def _update_status(agent, queue, uid, callback_method):
        agent.action({
            ...
            callback=('asterisk_base_queues.queue', callback_method),
            passback={
                'uid': uid,
                'queue': queue.id,
        })
    ...
    def update_status_response(self, data):
        ...
        this_queue = self.env['asterisk_base_queues.queue'].browse(
            data.get('passback').get('queue')).sudo()
