...
class AvaxAccountSendWizard(models.TransientModel):
    ...
    account_id = fields.Many2one("Conta de Origem",
        'avax.account', default=_get_default_account_id)

    balance = fields.Float(string="Saldo", related='account_id.balance')
    password = fields.Char("Senha da Carteira")
    account_to_id = fields.Many2one("Conta de Destino", 'avax.account')
    address = fields.Char(string="Endereço de Destino")
    amount = fields.Integer(string="Valor")
