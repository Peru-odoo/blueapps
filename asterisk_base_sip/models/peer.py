...
class SipPeer(models.Model):
    ...
    agent = fields.Many2one(related='server.agent')
    ...
    canreinvite = fields.Selection([('yes', 'Yes'), ('no', 'No')],
                                   string='Can reinvite', default='no')
    ...
    language = fields.Char(size=2, default=lambda x: x._get_default_channel_language())
    ...
    inbound_auth_nonce_lifetime = fields.Char(string='Nonce Lifetype')
    ...
    @api.constrains(
        'defaultuser', 'fromdomain', 'fromuser', 'callbackextension', 'host')
    def _check_trunk_doubles(self):
        for rec in self:
            if rec.peer_type == 'user':
                continue
            doubles = self.env['asterisk_base_sip.peer'].search_count([
            ...
            if doubles > 1:
                raise ValidationError(
                    _('You can\'t create another trunk with the same values'))
    ...
    def create(self, vals):
        ...
        if res.user and not self.env.context.get('no_create_channel'):
    ...
    def create_user_channel(self):
        self.ensure_one()
        # Check that there is not yet a SIP peer
        user_chan = self.env['asterisk_common.user_channel'].search([
            ('agent', '=', self.agent.id),
            ('channel', '=ilike', '{}/{}'.format(
                self.channel_type, self.name))])
        if user_chan:
            if user_chan.asterisk_user == self.user:
                # Just connect them together
                user_chan.sip_peer = self.id
            else:
                # Manual intervention is required
                raise ValidationError(
                    'SIP/{} is already added to user {}!'.format(
                        self.name, user_chan.user.name))
        else:
            # No existing channels found, create one
            self.env['asterisk_common.user_channel'].create({
                    'asterisk_user': self.user.id,
                    'sip_peer': self.id,
                    'originate_context': self.context,
    ...
    def unlink_user_channel(self):
        self.ensure_one()
        self.env['asterisk_common.user_channel'].search(
            [('sip_peer', '=', self.id)]).with_context(from_peer=True).unlink()
        if not self.env.context.get('no_build_conf'):
            self.env['asterisk_common.user'].build_conf()
    ...
    def write(self, vals):
        ...
        if 'user' in vals:
            for rec in self:
                if rec.user and not self.env.context.get('no_create_channel'):
    ...
    def _get_default_channel_language(self):
        if self.env.context.get('install_mode') or self.env.context.get(
                'module') == 'asterisk_base_sip':
            return 'en'
        return self.env['asterisk_common.settings'].get_param(
            'default_sip_channel_language')
    ...
    def build_extension(self):
        ...
        return self.env['asterisk_base.conf_template'].render_template(
            'sip_extension', self.server.id, {'rec': self})

    def build_conf(self):
        self.build_extensions_conf()
        self.build_sip_conf()
        self.build_pjsip_conf()
        self.env['asterisk_base_sip.peer_template'].build_conf()
    ...
    def build_extensions_conf(self):
        ...
        for rec in self.search([], order='id'):
            ...
            rendered = self.env['asterisk_base.conf_template'].render_template(
                'sip_peer_context', rec.server.id, {'rec': rec})
            conf_dict[rec.server.id] += '{}'.format(rendered)
        # Create conf files
        for server_id in conf_dict.keys():
            ...
            # Set conf content
            conf.content = conf_dict[server_id]
            ...
            for rec in records:
                ...
                rendered = self.env[
                    'asterisk_base.conf_template'].render_template(
                        'sip_peer', rec.server.id,
                        {'rec': rec, 'channel_vars': channel_vars})
                conf_dict[rec.server.id] += '{}'.format(rendered)
            # Create conf files
            for server_id in conf_dict.keys():
                # First try to get existing conf
                conf = self.env['asterisk_base.conf'].get_or_create(
                    server_id, conf_name)
                # Set conf content
                conf.content = '{}'.format(conf_dict[server_id])
    ...
    def update_status_request(self, notify_uid=None):
        for rec in self:
            rec.server.agent.action(
                {
                    'Action': 'SIPshowpeer',
                    'Peer': rec.name
                },
                notify=True,
                callback=('asterisk_base_sip.peer', 'update_status_response'),
                passback={
                    'res_id': rec.id,
                    'notify_uid': notify_uid or rec.env.uid
                })

    @api.model
    def update_status_response(self, response):
        peer_id = response['passback']['res_id']
        notify_uid = response['passback']['notify_uid']
        if response.get('result'):
            # Check for Action error
            if response['result'][0].get('Response') == 'Error':
                self.env.user.asterisk_notify(
                    response['result'][0]['Message'],
                    warning=True,
                    uid=notify_uid)
                return True
            get = response['result'][0].get
            try:
            ...
            # Asterisk error
            self.env.user.asterisk_notify(
                'Asterisk error: %s', response['error']['message'],
                uid=notify_uid)
            return False
