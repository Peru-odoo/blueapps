...
class SipRoute(models.Model):
    ...
    translations_count = fields.Integer(compute='_get_translations_count')
    note = fields.Text()
    server = fields.Many2one(
        comodel_name='asterisk_base.server', required=True,
        default=get_default_server)
    groups = fields.Many2many(
        comodel_name='asterisk_base_sip.route_group',
    ...
    sip_headers = fields.Text(string='SIP Headers')
    ...
    def _get_translations_count(self):
        for rec in self:
            rec.translations_count = len(
                rec.dst_translations) + len(rec.src_translations)
    ...
    def _get_dialplan(self):
        for rec in self:
            rec.built_dialplan = self.env[
                'asterisk_base.conf_template'].render_template(
                'route', rec.server.id, {'rec': rec})
