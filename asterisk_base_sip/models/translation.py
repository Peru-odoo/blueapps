...
class Translation(models.Model):
    ...
    routes = fields.One2many('asterisk_base_sip.route',
                             compute='_get_routes')
    routes_count = fields.Integer(compute='_get_routes_count')

    def _get_name(self):
        for rec in self:
            if rec.translation_type == 'src':
    ...
    def _get_src_result(self):
        for rec in self:
            ...
            if rec.src_channel:
                res += rec.src_channel + ' '
            if rec.src_user:
                res += rec.src_user.name + ' '
            res += '=> '
            if rec.src_replace:
                rec.src_result = res + rec.src_replace
            else:
                res += rec.src_prefix if rec.src_prefix else ''
                if rec.src_skip:
                    res += '${{EXTEN:{}'.format(rec.src_skip)
                    if rec.src_skip_length:
                        res += ':{}'.format(rec.src_skip_length)
                    res += '}'
                rec.src_result = res
    ...
    def _get_dst_result(self):
        for rec in self:
            if rec.dst_replace:
                rec.dst_result = '{} => {}'.format(rec.dst, rec.dst_replace)
            else:
                res = '{} => {}'.format(
                    rec.dst, rec.dst_prefix) if rec.dst_prefix else ''
                res += '${EXTEN'
                if rec.dst_skip:
    ...
    def _check_skip(self):
        for rec in self:
            if (rec.dst_skip and not rec.dst_skip.isdigit()) or (
                    rec.src_skip and not rec.src_skip.isdigit()):
                raise ValidationError(
                    _('Skip digits must be an integer value!'))
            if (rec.dst_skip_length and not rec.dst_skip_length.isdigit()) or (
                    rec.src_skip_length and not rec.src_skip_length.isdigit()):
                raise ValidationError(
    ...
    def create(self, vals):
        ...
        if rec and not self.env.context.get('no_build_conf'):
            self.env['asterisk_base_sip.route'].build_conf()
